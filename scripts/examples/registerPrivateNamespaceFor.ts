/**
 * Script to register a private namespace for another address (OWNER-only, onboarding period only)
 *
 * USAGE:
 * Run the script with:
 * `npx hardhat run scripts/examples/registerPrivateNamespaceFor.ts --network <network_name>`
 *
 * EXAMPLE:
 * To register a private namespace on Sepolia:
 * `npx hardhat run scripts/examples/registerPrivateNamespaceFor.ts --network sepolia`
 *
 * REQUIRED SETUP:
...
async function main() {
  ...
  // Register namespace
  console.log(`Registering private namespace for creator: ${GREEN}${creatorAddress}${RESET}`);
  console.log(`Namespace: ${GREEN}${namespace}${RESET}\n`);

  const registerTx = await xns.connect(signer).registerPrivateNamespaceFor(
    creatorAddress,
    namespace,
    pricePerName,
    {
